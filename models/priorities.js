...
module.exports = {
    ...
    update: (priority) => {
        ...
        return db(table).where(id, req.params.id).update({ priority_nm: priority.name, priority_value: priority.value }, allFields);
    },

    delete: (id) => {

        return db(table).where(id, req.params.id).del();
    }
};